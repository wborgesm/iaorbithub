...
const SESSION_DIR = path.join(process.cwd(), 'data', 'orbit-business-whatsapp')
const CLIENT_ID = 'orbit-jarvis-business'
...
async function onSyncTimeout(): Promise<void> {
  ...
  console.warn('[whatsappBusiness] Timeout na sincronização — reinício automático')
...
async function transcribeAudio(audioBuffer: Buffer, mimeType: string): Promise<string | null> {
  try {
    ...
    console.warn('[whatsappBusiness] transcribeAudio erro:', (err as Error).message)
    return null
  }
}
...
function attachListeners(c: Client): void {
  c.on('qr', async (qr: string) => {
    ...
    console.log('[whatsappBusiness:orbit-business] QR — escaneia com o WhatsApp BUSINESS')
  })

  c.on('authenticated', () => {
    state = 'connecting'
    qrDataUrl = null
    console.log('[whatsappBusiness] Autenticado, a sincronizar…')
    clearSyncTimeout()
  ...
  c.on('ready', () => {
    ...
    console.log(`[whatsappBusiness:orbit-business] Ligado${phone ? ` (+${phone})` : ''}`)
  })
  ...
  c.on('auth_failure', (msg: string) => {
    ...
    console.error('[whatsappBusiness] auth_failure:', lastError)
  })

  c.on('disconnected', (reason: string) => {
    clearSyncTimeout()
    console.warn('[whatsappBusiness] Desligado:', reason)
    state = 'idle'
    qrDataUrl = null
    phone = null
    client = null
        ...
        try {
          ...
          if (media?.data) {
            ...
            if (transcription) {
              ...
              console.log(`[whatsappBusiness] Voz transcrita de ${from}: ${transcription.slice(0, 100)}`)
            }
          }
        } catch (voiceErr) {
          console.warn('[whatsappBusiness] Erro ao transcrever voz:', (voiceErr as Error).message)
        }
      }
...
export async function resumeWhatsAppWebIfPossible(): Promise<void> {
  ...
  try {
    ...
    console.warn('[whatsappBusiness] Retoma automática falhou:', (e as Error).message)
  }
}
...
export async function restartWhatsAppWeb(): Promise<void> {
  console.log('[whatsappBusiness:orbit-business] Reinício pedido')
  await destroyClient(false)
  await killOrphanChrome()
  await startWhatsAppWeb()
}
...
export async function sendViaWhatsAppWeb(
  ...
  if (state !== 'ready' || !client) {
    return {
      ok: false,
      error: 'WhatsApp Business não ligado. Abre /orbit → Configuração → WhatsApp Business → Ligar (QR).',
    }
  }
...
export async function getRecentWhatsAppMessages(
  ...
  if (state !== 'ready' || !client) {
    return { ok: false, error: 'WhatsApp Business não ligado.' }
  }
  try {
...
export async function sendViaWhatsAppByName(
  ...
  if (state !== 'ready' || !client) return { ok: false, error: 'WhatsApp Business não ligado.' }
  try {
    ...
    return { ok: false, error: `Conversa com "${name}" não encontrada no WhatsApp Business.` }
  } catch (err) {
    return { ok: false, error: err instanceof Error ? err.message : 'Erro ao enviar' }
  }
}
